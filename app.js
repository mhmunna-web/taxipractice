  ...
  pageEl.innerHTML = `
    ...
    <div class="grid">
      ...
      <div class="card">
        ...
        <p>MCQ practice with correct/wrong + explanation.</p>
        <div class="row">
      ...
      <div class="card">
        ...
        <h3>⏱️ Real Exam</h3>
        <p>We will add exam mode after everything works.</p>
        <div class="row">
          <span class="small muted">Coming soon</span>
          <a class="btn" href="#/resources">Info</a>
        </div>
      </div>
    </div>
  `;
...
function renderStudy() {
  ...
  const listHtml = chapters.map(c => `
    <div class="item ${c.id===selected.id ? "active" : ""}" data-ch="${escapeHtml(c.id)}">
      ...
      <div class="kicker">${escapeHtml(c.id)} • ${(c.items?.length||0)} lines</div>
    </div>
  `).join("");
  ...
  pageEl.innerHTML = `
    ...
    <p class="lead">Select a chapter from left.</p>
...
function renderPractice() {
  ...
  const all = getAllQuestions();
  if (!all.length) {
    pageEl.innerHTML = `<h1 class="h1">📝 Practice</h1><p class="lead">No questions found in data/questions.json</p>`;
    return;
  }
...
function renderResources() {
  ...
  pageEl.innerHTML = `<h1 class="h1">🔗 Resources / FAQ</h1><p class="lead">Add useful links later.</p>`;
}
...
searchEl.addEventListener("keydown", (e) => {
  ...
  if (!q) return;
  alert("Search feature will be added later. You typed: " + q);
});
